...
common_params_context common_params_parser_init(common_params & params,
    ...
    // DDS options
    add_opt(common_arg({ "--enable-dds" }, { "--disable-dds" }, "enable DDS (CycloneDDS) transport",
                       [](common_params & params, bool value) { params.enable_dds = value; })
                .set_examples({ LLAMA_EXAMPLE_SERVER })
                .set_env("LLAMA_ARG_ENABLE_DDS"));
    add_opt(common_arg({ "--dds-domain" }, "DOMAIN", "DDS domain ID (default: 0)",
                       [](common_params & params, int value) { params.dds_domain = value; })
                .set_examples({ LLAMA_EXAMPLE_SERVER })
                .set_env("LLAMA_ARG_DDS_DOMAIN"));
    add_opt(common_arg({ "--dds-timeout" }, "SECONDS", "DDS request timeout in seconds (default: 60)",
                       [](common_params & params, int value) { params.dds_timeout_secs = value; })
                .set_examples({ LLAMA_EXAMPLE_SERVER })
