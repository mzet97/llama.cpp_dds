...
def plot_comparison(df, output_dir):
    ...
    # 3. Bar Chart: p50 Median Latency Comparison
    plt.figure(figsize=(10, 6))

    dds_p50  = df[df['protocol'] == 'DDS']['p50'].values
    http_p50 = df[df['protocol'] == 'HTTP']['p50'].values

    if len(dds_p50) == len(prompts) and len(http_p50) == len(prompts):
        plt.bar(x - width/2, dds_p50,  width, label='DDS')
        plt.bar(x + width/2, http_p50, width, label='HTTP')

        plt.xlabel('Prompt Complexity')
        plt.ylabel('Median Latency p50 (ms)')
        plt.title('DDS vs HTTP: Median Latency (p50)')
        plt.xticks(x, prompts)
        plt.legend()
        plt.grid(True, axis='y')

        for i, v in enumerate(dds_p50):
            plt.text(i - width/2, v, f'{v:.1f}', ha='center', va='bottom')
        for i, v in enumerate(http_p50):
            plt.text(i + width/2, v, f'{v:.1f}', ha='center', va='bottom')

        plt.savefig(os.path.join(output_dir, 'p50_comparison.png'))
        print(f"Saved p50_comparison.png to {output_dir}")

    # 4. Grouped Bar Chart: Mean / p50 / p95 / p99 side-by-side per protocol
    fig, axes = plt.subplots(1, len(prompts), figsize=(5 * len(prompts), 6), sharey=False)
    if len(prompts) == 1:
        axes = [axes]

    metric_cols = ['mean', 'p50', 'p95', 'p99']
    x_m = np.arange(len(metric_cols))
    w_m = 0.35

    for ax, prompt_name in zip(axes, prompts):
        row_dds  = df[(df['protocol'] == 'DDS')  & (df['prompt_type'] == prompt_name)]
        row_http = df[(df['protocol'] == 'HTTP') & (df['prompt_type'] == prompt_name)]
        if row_dds.empty or row_http.empty:
            continue
        vals_dds  = [row_dds[m].values[0]  for m in metric_cols]
        vals_http = [row_http[m].values[0] for m in metric_cols]

        ax.bar(x_m - w_m/2, vals_dds,  w_m, label='DDS')
        ax.bar(x_m + w_m/2, vals_http, w_m, label='HTTP')
    ...
    fig.suptitle('DDS vs HTTP: All Percentiles by Prompt Complexity')
    fig.tight_layout()
    fig.savefig(os.path.join(output_dir, 'all_percentiles.png'))
    print(f"Saved all_percentiles.png to {output_dir}")
